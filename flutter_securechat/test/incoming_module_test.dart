...
void main() {
  ...
  test(
    'a call media key is routed to the call manager, not the chat',
    () async {
      final fixture = await _Fixture.open();
      addTearDown(fixture.close);
      final mediaKey = CallMediaKey.generate(callId: 'call-1');
      final envelope = await fixture.crypto.encryptDirect(
        recipientId: 'me',
        plaintext: mediaKey.encode(),
      );
      ...
      expect(fixture.receivedMediaKeys, [('alice', mediaKey.encode())]);
      // Anahtar bir sohbet mesaji degildir; veritabanina yazilmamali.
      expect(await fixture.database.messages.getMessageCount('alice'), 0);
    },
  );

  test('a call media key addressed to someone else is ignored', () async {
    final fixture = await _Fixture.open();
    addTearDown(fixture.close);
    final mediaKey = CallMediaKey.generate(callId: 'call-1');
    final envelope = await fixture.crypto.encryptDirect(
    ...
    expect(fixture.receivedMediaKeys, isEmpty);
    expect(await fixture.database.messages.getMessageCount('alice'), 0);
  });
...
class _Fixture {
  ...
  static Future<_Fixture> open() async {
    ...
    return _Fixture(directory, database, crypto, signaling, handler, mediaKeys);
  }
