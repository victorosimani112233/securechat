...
/** Binary-level production boundary; evaluated before config or listeners. */
object ProductionDeploymentPolicy {
    fun validate(environment: Map<String, String> = System.getenv()) {
        ...
        val publicSocket = environment["BOT_PUBLIC_SOCKET"] ?: "/run/bot/bot-public.sock"
        val adminSocket = environment["BOT_ADMIN_SOCKET"] ?: "/run/bot/bot-admin.sock"
        for (socket in listOf(publicSocket, adminSocket)) {
            require(socket.startsWith("/run/bot/") && !socket.contains("..")) {
                "Bot Unix sockets must stay inside /run/bot"
            }
        }
    }
