...
class EdDsaJwtVerifier(private val clientLookup: (kid: String) -> AuthenticatedClient?) {
    ...
    fun verify(bearerToken: String, requestBody: ByteArray): Result {
        ...
        val jws = try {
            ...
            return Result.Fail(Reason.MALFORMED_TOKEN, "JWT parse hatasi")
        }
        ...
        val client = clientLookup(kid)
            ?: return Result.Fail(Reason.UNKNOWN_OR_REVOKED_CLIENT, "kid bilinmiyor veya revoked")

        // 3. JDK native Ed25519 verify
        val sigOk = try {
            verifyEd25519(client.publicKey, jws)
        } catch (e: Exception) {
            log.warn("[JWT] Ed25519 verify exception: {}", e.javaClass.simpleName)
            false
        }
        if (!sigOk) {
        ...
        val claims: JWTClaimsSet = try {
            ...
            return Result.Fail(Reason.MALFORMED_TOKEN, "Claims parse hatasi")
        }
