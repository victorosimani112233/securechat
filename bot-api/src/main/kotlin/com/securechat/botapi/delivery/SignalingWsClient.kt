 ...
 *
 *  - URL: ${SIGNALING_WS_URL}?userId=<botUserId>&token=<accessJwt>
...
object SignalingWsClient {
    ...
    private suspend fun reconnectLoop() {
        ...
        while (!stopping) {
            try {
                val token = BotJwtMinter.issueAccessToken(BotIdentity.get().botUserId)
                val url = "${BotApiConfig.signalingWsUrl}?userId=${BotIdentity.get().botUserId}&token=$token"
                log.info("[WSClient] Baglaniliyor: {}", url.substringBefore("?token") + "?token=***")
                ...
                log.warn("[WSClient] Baglanti hatasi: {}", e.message)
            }
    ...
    private class BotWsListener : WebSocketListener() {
        ...
        override fun onClosing(webSocket: WebSocket, code: Int, reason: String) {
            log.warn("[WSClient] WS kapanmak uzere: code={}, reason={}", code, reason)
            connected = false
            try { webSocket.close(code, reason) } catch (_: Exception) {}
        }
        override fun onClosed(webSocket: WebSocket, code: Int, reason: String) {
            log.warn("[WSClient] WS kapali: code={}, reason={}", code, reason)
            connected = false
        }
        override fun onFailure(webSocket: WebSocket, t: Throwable, response: Response?) {
            log.warn("[WSClient] WS hata: {}", t.message)
            connected = false
        }
    }
}
